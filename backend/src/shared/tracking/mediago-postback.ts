...
/** Mediago TRACKING_ID from click record (click_id / trackingid on LP URL). */
export function resolveMediagoTrackingId(click: Click): string {
...
export function resolveMediagoAccountName(
  ...
  const fromEnv = (envAccountName || process.env.MEDIAGO_ACCOUNT_NAME || '').trim();
  return fromEnv;
}

export function resolveMediagoAdId(click: Click): string {
